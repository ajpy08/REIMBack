...
//  GET TIPO RELACION
// ===========================================
app.get('/Tipo/:tipo/', mdAutenticacion.verificaToken, (req, res) => {
    let tipo = req.params.tipo;
    tipoRelacion.find({ 'clave': tipo }, (err, tipo) => {
        if (err) {
            return res.status(500).json({
                ok: false,
                mensaje: 'Error al buscar Tipo Relacion',
                errors: err
            });
        }
...
//  GENERAR PDF NOTA
// ===========================================
app.get('/pdFNota/:id', mdAutenticacion.verificaToken, (req, res) => {
    let id = req.params.id;
    let nombrePdf = '';
    let DTO = 0;
    async function ok() {
        ...
        if (Cf === null) {
            ...
            if (Mt === null) {
                ...
                if (Cl === null) {
                    ...
                    const Tipo = await ConsultaPDF.Tipo(Cf.tipoRelacion);
                    if (Tipo === null) {
                        return res.status(400).json({
                            ok: false,
                            mensaje: 'Error al buscar Tipo Relacion para generar PDF',
                            errors: { message: 'Error al buscar Tipo Relacion para generar PDF' }
                        });
                    } else {
                        const SubT = await funcion.cortado(Cf.subtotal, 2, true);
                        const Total_I_R = await funcion.cortado(Cf.totalImpuestosRetenidos, 2, true);
                        ...
                        const selloE = await funcion.cadenaOriginalComplemetoPDF(Cf.selloEmisor, 2);
                        const selloT = await funcion.cadenaOriginalComplemetoPDF(Cf.selloSat, 2);
                        const cfdisRelacionados = await funcion.cfdiRel(Cf.conceptos);
                        return {
                            Cf, Mt, Cl, Us, SubT, Total_I_R, Total_I_T, Total, letraT, cadena, QrG, selloE, selloT, cfdisRelacionados, Tipo
                        }
                    }
                }
            }
    ...
    ok().then(ress => {
        let TipoRelacionCD = '';
        for (const t of ress.Tipo) {
            TipoRelacionCD = t.clave + ' - ' + t.descripcion;
        }
            ...
            content: [
                ...
                // },
                {
                    ...
                    table: {
                        ...
                        widths: ['*', '*'],
                        body: [
                            [{ text: 'CFDIS RELACIONADOS', fillColor: '#b2b4b5', alignment: 'left' }, { text: '', fillColor: '#b2b4b5', alignment: 'right' }],
                            [
                                {
                                    type: 'none',
                                    ul: [
                                        { text: [ress.cfdisRelacionados] },
                                    ], alignment: 'left', fontSize: 8
                                },
                                {
                                    text: [''], alignment: 'right', margin: [18, 15, 10, 0]
...
app.get('/envioCorreo/:correo&:archivo&:nombre', mdAutenticacion.verificaToken, (req, res) => {
    ...
    envio().then(() => {
        console.log('archivo enviado :' + archivoPDF + ',' + archivoXML + 'Enviado a: ' + correo);
        res.status(200).json({
            ok: true,
            archivos: archivos
        });
