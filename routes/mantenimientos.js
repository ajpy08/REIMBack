...
//   const mantenimientos = mantenimientosController.getMantenimientos(req, res);
//   mantenimientos.then(mantenimientos => {
//     console.log(mantenimentos.mantenimientos);
//     return res.status(200).json({
//       ok: true,
//       mantenimientos,
//       totalRegistros: mantenimientos.length
...
app.post('/mantenimiento', mdAutenticacion.verificaToken, (req, res) => {
  const newMant = mantenimientosController.insertMaterial(req, res);
...
app.delete('/mantenimiento/:id', async(req, res) => {
  ...
  if (mantenimiento && mantenimiento._id) {
    ...
    //    const maniobra = await maniobrasController.updateEstatus(mantenimiento.maniobra, estatus);
...
// folder = `mantenimientos/${mante._id}/`;
...
// const mantenimientos = await mantenimientosController.getMantenimientos({ params: { maniobra: mante.maniobra } }, res);
// mantenimientos.forEach(m => {
//   if (m._id !== mante._id)
//     losdemasFinalizados = m.finalizado && losdemasFinalizados;
// });

// if (losdemasFinalizados !== true) estatus = "LAVADO_REPARACION";



//   Maniobra.updateOne({ '_id': new mongoose.Types.ObjectId(mante.maniobra) }, {
//     $set: {
//       'estatus': estatus,
//     }
//   }, function (err) {
...
//   return res.status(500).json({
//     ok: false,
//     mensaje: 'Hubo un error al intentar consultar la Manibra asociada',
//     errors: error
//   });
// });
